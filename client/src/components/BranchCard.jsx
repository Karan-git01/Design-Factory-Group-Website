  ...
  return (
    <Link to={`/branches/${slug}`} className="group block">
      <div className="relative aspect-[16/9] overflow-hidden rounded-3xl border border-border bg-card">
        ...
        {isMain && (
          <span className="label-caps absolute left-4 top-4 rounded-full bg-copper px-3 py-1.5 text-primary-foreground shadow-sm">
            Main Branch
          </span>
        )}
      </div>
      ...
      <div className="mt-5 flex items-start justify-between gap-4">
        <div className="min-w-0">
          <h3 className="font-display truncate text-2xl tracking-tight">{name}</h3>
          <p className="mt-2 line-clamp-2 max-w-sm text-sm text-muted-foreground">
            {address}
          </p>
        </div>

        <span className="mt-1 grid h-10 w-10 shrink-0 place-items-center rounded-full border border-border text-copper transition-all duration-300 group-hover:border-copper group-hover:bg-copper group-hover:text-background">
          <ArrowUpRight
            size={16}
