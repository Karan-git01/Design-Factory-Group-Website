...
const CONTACT_DETAILS = [
  { Icon: MapPin, label: "1200 Bower Street, Suite 4", href: null },
  { Icon: Phone, label: "+1 (415) 555 0148", href: "tel:+14155550148" },
  { Icon: Mail, label: "studio@designfactorygroup.com", href: "mailto:studio@designfactorygroup.com" },
];
          ...
          <li key={to + label}>
            ...
            >
              <span className="relative">
                {label}
                <span className="absolute -bottom-0.5 left-0 h-px w-0 bg-copper transition-all duration-300 group-hover/link:w-full" />
              </span>
            </Link>
          </li>
        ))}
    ...
    <footer className="border-t border-border bg-cream-alt">
      <div className="mx-auto max-w-7xl px-5 pt-16 pb-8 sm:px-8 md:pt-24 md:pb-10">
        ...
        <div className="flex flex-col items-start justify-between gap-8 border-b border-border pb-14 md:flex-row md:items-end">
          ...
          >
            Start a project
            <span className="grid h-6 w-6 place-items-center rounded-full bg-copper text-background">
              <ArrowUpRight
                size={13}
        ...
        <div className="grid gap-10 pt-14 pb-12 md:grid-cols-[1.2fr_1fr_1fr_1fr] md:gap-8">
          <div>
            ...
            <p className="mt-4 max-w-[26ch] text-sm text-foreground/70">
              A design and build studio shaping considered, high-end
              residential work from concept through completion.
            </p>
              ...
              {CONTACT_DETAILS.map(({ Icon, label, href }) => (
                <li key={label}>
                  {href ? (
                    ...
                    >
                      <Icon size={15} className="mt-0.5 flex-none text-muted-foreground/70 transition-colors duration-300 group-hover/contact:text-copper" />
                      {label}
                    </a>
                  ) : (
                    <span className="flex items-start gap-2.5 text-sm text-foreground/80">
                      <Icon size={15} className="mt-0.5 flex-none text-muted-foreground/70" />
                      {label}
                    </span>
                  )}
                </li>
